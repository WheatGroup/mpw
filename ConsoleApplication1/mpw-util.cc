...
void mpw_push_buf(uint8_t **const buffer, size_t *const bufferSize, const void *pushBuffer, const size_t pushSize) {
    ...
    *bufferSize += pushSize;
    uint8_t *resizedBuffer = (uint8_t *)realloc( *buffer, *bufferSize );
    if (!resizedBuffer) {
...
const char *mpw_hex(const void *buf, size_t length) {
    ...
    // FIXME
    if (!mpw_hex_buf) {
        mpw_hex_buf = (char **)malloc( 10 * sizeof( char * ) );
        for (uint8_t i = 0; i < 10; ++i)
            mpw_hex_buf[i] = NULL;
    }
    mpw_hex_buf_i = (mpw_hex_buf_i + 1) % 10;

    mpw_hex_buf[mpw_hex_buf_i] = (char *)realloc( mpw_hex_buf[mpw_hex_buf_i], length * 2 + 1 );
