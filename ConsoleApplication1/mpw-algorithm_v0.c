...
static const char *mpw_passwordForSite_v0(const uint8_t *masterKey, const char *siteName, const MPSiteType siteType, const uint32_t siteCounter,
    ...
    const char *template1 = mpw_templateForType_v0( siteType, htons( sitePasswordSeed[0] ) );
    ...
    for (size_t c = 0; c < strlen( template1 ); ++c) {
        sitePassword[c] = mpw_characterFromClass_v0( template1[c], htons( sitePasswordSeed[c + 1] ) );
        trc( "class %c, index %u (0x%02X) -> character: %c\n",
                template1[c], htons( sitePasswordSeed[c + 1] ), htons( sitePasswordSeed[c + 1] ), sitePassword[c] );
    }
    mpw_free( sitePasswordSeed, sizeof( sitePasswordSeed ) );
